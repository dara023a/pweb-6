...
function twitchAuth(options) {
  return async (c, next) => {
    ...
    const auth = new AuthFlow({
      client_id: options.client_id || env(c).TWITCH_ID,
      client_secret: options.client_secret || env(c).TWITCH_SECRET,
...
async function refreshToken(client_id, client_secret, refresh_token) {
  ...
  if ("message" in response) {
    throw new HTTPException3(400, { message: response.message });
  }
  return response;
}
  ...
  if (!res.ok) {
    try {
      ...
      if (errorResponse && typeof errorResponse === "object" && "message" in errorResponse) {
        throw new HTTPException4(400, { message: errorResponse.message });
      }
    } catch (e) {
...
async function validateToken(token) {
  ...
  if ("status" in response) {
    throw new HTTPException5(400, { message: response.message });
  }
  return response;
}
export {
